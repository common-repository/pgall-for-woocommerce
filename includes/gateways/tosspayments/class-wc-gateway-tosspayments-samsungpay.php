...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	if ( ! class_exists( 'WC_Gateway_TossPayments_SamsungPay' ) ) {

		class WC_Gateway_TossPayments_SamsungPay extends WC_Gateway_TossPayments {

			public function __construct() {
				$this->id = 'tosspayments_samsungpay';

				parent::__construct();

				if ( empty( $this->settings['title'] ) ) {
					$this->title       = __( '삼성페이', 'pgall-for-woocommerce' );
